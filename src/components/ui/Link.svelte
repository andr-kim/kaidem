<script lang="ts">
  ...
  const base =
    "inline-flex items-center transition-colors duration-200 font-medium focus-visible:outline-none";

  const variants: Record<NonNullable<Props["variant"]>, string> = {
    link: "py-2 text-blue-600 hover:text-blue-700 underline-offset-4 hover:underline",
    navlink:
      "p-2 text-muted-foreground hover:text-foreground data-[active=true]:text-foreground",
    button:
